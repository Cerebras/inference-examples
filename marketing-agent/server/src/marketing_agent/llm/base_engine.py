...
def parse_obj_response(response_model: type[BaseModel], content: str):
    ...
    try:
        if "```json" in content:
            ...
            raise ValueError("No code block found in the content")

        json_end = content.find("```", json_start)
        if json_end == -1:
            raise ValueError("Closing code block not found")
        ...
        raise ValueError(f"Failed to validate against response model: {str(e)}")
    except Exception as e:
        logging.error(f"Unexpected error in parse_obj_response: {str(e)}")
